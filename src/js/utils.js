...
* @param {Array<HTMLElement>} $elements - An array of DOM elements to attach the event listener.
...
/**
 * Generates a greeting message based on the current hours of the day.
 * 
...
let /**{HTMLElement | undefiend} */ $lastActiveNavItem;
...
const activeNotebook = function () {
    // console.log("This is the div", this);
    $lastActiveNavItem?.classList.remove('active');
    // console.log($lastActiveNavItem);
    this.classList.add('active') // this: $navItem
    $lastActiveNavItem = this; // this: $navItem
}

/**
 * Makes a DOM element editable by setting the 'contenteditable' attributes to true and focusing on it. 
...
/**
 * Finds the index of the notebook in an array of the notebooks based on its ID.
 * 
 * @param {object} db - The object conataining an array of notebooks.
 * @param {string} notebookId - The ID of the notebook to find.
 * @returns {number} The index of the found notebook, or -1 id not found 
*/
 ...
 * 
 * @param {number} millisecond - The timestamp in millisecond to convert.
 * 
 * @returns {string} A string representing the relative time (e.g. "Just now," "5 min ago," "3 hours ago," "2 days ago").
*/

const getRelativeTime = function (millisecond) {
    const /**{Number} */ currentTime = new Date().getTime();

    const /**{Number} */ minute = Math.floor((currentTime - millisecond) / 1000 / 60);
 ...
 * @returns {number} The index of the found not, or -1 if not found.
*/
